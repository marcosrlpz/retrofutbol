...
// ✅ Botón logout en verde como el resto de la navbar
...
const MobileLeagueBtn = styled.button`
  width: 100%; text-align: left;
  padding: 0.9rem 1.5rem;
  font-size: 0.85rem; font-weight: 700;
  text-transform: uppercase; letter-spacing: 0.05em; color: #2d4a2d;
  display: flex; justify-content: space-between;
`;
const MobileTeams = styled.div`display: ${({ $open }) => $open ? "block" : "none"}; background: #ede8dc;`;
const MobileTeamLink = styled(Link)`
  display: block; padding: 0.6rem 2rem;
  font-size: 0.85rem; color: #5a6b4a;
  &:hover { color: #c9a84c; }
`;
...
const Navbar = () => {
  ...
  return (
    <>
      <TopBar>Cada dia publicamos nuevas camisetas · <span>Envio gratis en pedidos superiores a 75 euros!</span></TopBar>
      <Header>
        <Container>
          <Brand to="/">
          ...
          <Actions>
            {isAuthenticated ? (
              <>
                {isAdmin && (
                  <IconBtn as={Link} to="/admin" title="Panel admin">
                    👑
                  </IconBtn>
                )}
                {/* ✅ Botón cerrar sesión en verde */}
